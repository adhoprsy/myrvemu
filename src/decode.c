...
static inline insn_t insn_itype_read(u32 data) { 
  return (insn_t) {
    .rs1 = RS1(data),
    .rd = RD(data),
    .imm = (i32)data >> 20, // auto sign extension after conversion to i32
  };
}
...
// c-type instructions are 16-bit, but we still pass u32 data to those 
// functions, u32->u16 automatically pick the lower 16-bits, so okay.
