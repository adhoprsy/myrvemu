...
inline i64 mulh(i64 a, i64 b) {
  int neg = ((a < 0) != (b < 0)); // one +, one -. aka mul result is neg
  u64 res = mulhu((a < 0 ? -a : a), (b < 0 ? -b : b));
  return neg ? ~res + (a * b == 0) : res; // ~0 = 1....1, +1 to 0000000
}

inline i64 mulhsu(i64 a, u64 b) {
  int neg = (a < 0);
  u64 res = mulhu((a < 0 ? -a : a), b);
  return neg ? ~res + (a * b == 0) : res;
}
