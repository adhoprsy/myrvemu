...
enum insn_type_t {
    ...
    num_insns,  // total number of instructions
};
typedef struct {
  i8 rd;
  i8 rs1, rs2, rs3;
...
enum exit_reason_t {
  ...
  indirect_branch,
  ecall,          // syscall
  interp,         // complex instru can't JIT, so just interp
};
...
#define CACHE_SIZE (64 * 1024 * 1024) // also size of jitcode
typedef struct { 
  u64 pc; // key
  u64 hot;
  u64 offset;
...
typedef struct {
  u8 *jitcode;  // a segment of mem, rwx, no recycle
  u64 offset;
  cache_item_t table[CACHE_ENTRY_SIZE]; // basically a hash table
} cache_t;
...
/*
 * decode.c
 * bitcode decode into instructions
*/
